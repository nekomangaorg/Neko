...
class GroupLibraryMangaUseCaseTest {
    ...
    fun `groupByDynamic groups items correctly by language`() {
        ...
        val multiLingualManga = mockLibraryMangaItem(mangaId = 4L, language = listOf("en", "ja"))

        val mangaList = listOf(englishManga1, englishManga2, japaneseManga, multiLingualManga)
        ...
        assertEquals(3, enCategory.libraryItems.size)

        val jaCategory = result.first { it.categoryItem.name == "ja" }
        assertEquals(2, jaCategory.libraryItems.size)

        // Assert properties of the generated CategoryItem
        val categoryGroup = result.first()
        assertEquals(0, categoryGroup.categoryItem.id) // The list index is used for id
        assertEquals(true, categoryGroup.categoryItem.isDynamic)
    }

    @Test
    fun `groupByDynamic groups items correctly by track status including fallbacks`() {
        // Arrange
        val trackedMangaCompleted = mockLibraryMangaItem(mangaId = 1L)
        val trackedMangaDropped = mockLibraryMangaItem(mangaId = 2L)
        val untrackedManga = mockLibraryMangaItem(mangaId = 3L)

        val mangaList = listOf(trackedMangaCompleted, trackedMangaDropped, untrackedManga)
        val currentGroup = LibraryGroup.ByTrackStatus
        val sortOrder = LibrarySort.Title
        val sortAscending = true
        val trackStatusMap = mapOf(1L to listOf("Completed"), 2L to listOf("Dropped"))
        ...
        val completedCategory = result.find { it.categoryItem.name == "Completed" }
        assertNotNull(completedCategory)
        assertEquals(1, completedCategory?.libraryItems?.size)
        assertEquals(1L, completedCategory?.libraryItems?.first()?.displayManga?.mangaId)

        val droppedCategory = result.find { it.categoryItem.name == "Dropped" }
        assertNotNull(droppedCategory)
        assertEquals(1, droppedCategory?.libraryItems?.size)
        assertEquals(2L, droppedCategory?.libraryItems?.first()?.displayManga?.mangaId)
        ...
        assertEquals(3L, untrackedCategory?.libraryItems?.first()?.displayManga?.mangaId)
    }
    ...
    fun `groupByUngrouped groups distinct items under a single all category`() {
        ...
        assertEquals(-1, categoryGroup.categoryItem.id)
        assertEquals(true, categoryGroup.categoryItem.isDynamic)
    ...
    fun `groupByCategory groups items correctly and skips empty system categories`() {
        ...
        val categoryEmpty = CategoryItem(id = 400, name = "400", sortOrder = LibrarySort.Title)
        ...
        val categories = listOf(category100, category200, categoryEmpty, systemCategoryEmpty)
        ...
        val group400 = result.find { it.categoryItem.id == 400 }
        assertNotNull(group400)
        assertEquals(0, group400?.libraryItems?.size)
