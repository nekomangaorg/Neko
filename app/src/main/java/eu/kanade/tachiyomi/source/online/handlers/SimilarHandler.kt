...
class SimilarHandler {
    ...
    suspend fun fetchRelated(
        ...
        return Ok(dbDto.relatedManga?.map { it.toSourceManga() } ?: emptyList())
    }
    ...
    suspend fun fetchRecommended(
        ...
        return Ok(dbDto.recommendedManga?.map { it.toSourceManga() } ?: emptyList())
    }
    ...
    private fun RelatedMangaDto.toSourceManga() =
        SourceManga(
            url = this.url,
    ...
    private suspend fun getMangadexMangaList(
        ...
        val responseBody =
            networkServices.service
                .search(ProxyRetrofitQueryMap(queryMap))
                .onError {
