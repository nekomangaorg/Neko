...
class ArtworkTypeMapping :
    SQLiteTypeMapping<ArtworkImpl>(
        ArtWorkPutResolver(),
        ArtWorkGetResolver(),
...
class ArtWorkPutResolver : DefaultPutResolver<ArtworkImpl>() {
    ...
    override fun mapToInsertQuery(obj: ArtworkImpl) =
        InsertQuery.builder().table(ArtworkTable.TABLE).build()

    override fun mapToUpdateQuery(obj: ArtworkImpl) =
        UpdateQuery.builder()
    ...
    override fun mapToContentValues(obj: ArtworkImpl) =
...
class ArtWorkGetResolver : DefaultGetResolver<ArtworkImpl>() {
    ...
    override fun mapFromCursor(cursor: Cursor): ArtworkImpl =
        ArtworkImpl(
...
class ArtWorkDeleteResolver : DefaultDeleteResolver<ArtworkImpl>() {
    ...
    override fun mapToDeleteQuery(obj: ArtworkImpl) =
        DeleteQuery.builder()
            .table(ArtworkTable.TABLE)
            .where("${ArtworkTable.COL_ID} = ?")
            .whereArgs(obj.id)
