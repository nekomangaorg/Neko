...
class SimilarTypeMapping :
    SQLiteTypeMapping<MangaSimilar>(
        SimilarPutResolver(),
        SimilarGetResolver(),
...
class SimilarPutResolver : DefaultPutResolver<MangaSimilar>() {
    ...
    override fun mapToInsertQuery(obj: MangaSimilar) = InsertQuery.builder().table(TABLE).build()

    override fun mapToUpdateQuery(obj: MangaSimilar) =
        UpdateQuery.builder().table(TABLE).where("$COL_ID = ?").whereArgs(obj.id).build()

    override fun mapToContentValues(obj: MangaSimilar) =
...
class SimilarGetResolver : DefaultGetResolver<MangaSimilar>() {
    ...
    override fun mapFromCursor(cursor: Cursor): MangaSimilar =
...
class SimilarDeleteResolver : DefaultDeleteResolver<MangaSimilar>() {
    ...
    override fun mapToDeleteQuery(obj: MangaSimilar) =
