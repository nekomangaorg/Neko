...
class CustomListTypeMapping : SQLiteTypeMapping<CustomListImpl>(
...
class CustomListPutResolver : DefaultPutResolver<CustomListImpl>() {
    ...
    override fun mapToInsertQuery(obj: CustomListImpl) = InsertQuery.builder()
        .table(CustomListTable.TABLE)
        .build()

    override fun mapToUpdateQuery(obj: CustomListImpl) = UpdateQuery.builder()
        .table(CustomListTable.TABLE)
        .where("${CustomListTable.COL_ID} = ?")
        .whereArgs(obj.id)
        .build()

    override fun mapToContentValues(obj: CustomListImpl) = ContentValues(3).apply {
...
class CustomListGetResolver : DefaultGetResolver<CustomListImpl>() {
    ...
    override fun mapFromCursor(cursor: Cursor): CustomListImpl = CustomListImpl(
...
class CustomListDeleteResolver : DefaultDeleteResolver<CustomListImpl>() {
    ...
    override fun mapToDeleteQuery(obj: CustomListImpl) = DeleteQuery.builder()
        .table(CustomListTable.TABLE)
        .where("${CustomListTable.COL_ID} = ?")
        .whereArgs(obj.id)
        .build()
