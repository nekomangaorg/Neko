...
class ScanlatorTypeMapping :
    SQLiteTypeMapping<ScanlatorImpl>(
...
class ScanlatorPutResolver : DefaultPutResolver<ScanlatorImpl>() {
    ...
    override fun mapToInsertQuery(obj: ScanlatorImpl) =
        InsertQuery.builder().table(ScanlatorTable.TABLE).build()

    override fun mapToUpdateQuery(obj: ScanlatorImpl) =
        UpdateQuery.builder()
    ...
    override fun mapToContentValues(obj: ScanlatorImpl) =
...
class ScanlatorGetResolver : DefaultGetResolver<ScanlatorImpl>() {
    ...
    override fun mapFromCursor(cursor: Cursor): ScanlatorImpl =
        ScanlatorImpl(
...
class ScanlatorDeleteResolver : DefaultDeleteResolver<ScanlatorImpl>() {
    ...
    override fun mapToDeleteQuery(obj: ScanlatorImpl) =
        DeleteQuery.builder()
            .table(ScanlatorTable.TABLE)
            .where("${ScanlatorTable.COL_ID} = ?")
            .whereArgs(obj.id)
