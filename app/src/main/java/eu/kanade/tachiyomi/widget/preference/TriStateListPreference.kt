...
constructor(
        ...
        defValue =
            when (defaultValue) {
                is Pair<*, *> -> {
                    ...
                    (defaultValue as? Collection<*>).orEmpty().mapNotNull { it as? String }.toSet()
            }
    }
        ...
        SummaryProvider<MatPreference> {
            ...
            allSelectionRes?.let { allRes ->
                when {
                    ...
                    allIsAlwaysSelected ->
                        includedStrings = includedStrings + context.getString(allRes)
                }
            }
            val excludedStrings =
                excludeKey
    ...
    override fun MaterialAlertDialogBuilder.setListItems() {
        ...
        val excludedSet =
            excludeKey?.let { sharedPreferences?.getStringSet(it, defValue) ?: defValue }.orEmpty()
        val allValue =
            intArrayOf(
                if (set.isEmpty()) {
        ...
        setTriStateItems(
            ...
            val selections =
                sels.filterIndexed { index, _ -> allSelectionRes == null || index != allPos }
            includedItems =
                selections
                    .mapIndexed { index, value ->
                        if (value == TriStateCheckBox.State.CHECKED.ordinal) index else null
                    }
                    .filterNotNull()
                    .map { entryValues[it] }
                    .toSet()
            excludedItems =
                selections
                    .mapIndexed { index, value ->
                        if (value == TriStateCheckBox.State.IGNORE.ordinal) index else null
                    }
                    .filterNotNull()
                    .map { entryValues[it] }
                    .toSet()
