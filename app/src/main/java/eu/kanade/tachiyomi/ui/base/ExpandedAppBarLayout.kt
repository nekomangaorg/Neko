...
@JvmOverloads
constructor(context: Context, attrs: AttributeSet? = null) : AppBarLayout(context, attrs) {
     ...
     *
     * use only on controller.onViewCreated that asynchronously loads the first set of items and
     * make false once the recycler has items
     */
    var lockYPos = false

    /**
     * A value used to determine the offset needed for a recycler to land just under the smaller
     * toolbar
     */
    ...
    fun getEstimatedLayout(
        ...
        return appBarHeight +
            (if (hasLargeToolbar) textHeight else 0) +
    ...
    /**
     * For smaller devices, update the big view (with the large title) to be a smaller font and less
     * padding
     */
    private fun shrinkAppBarIfNeeded(config: Configuration?) {
        ...
        if (config.screenHeightDp < 600) {
            ...
            bigTitleView.updateLayoutParams<MarginLayoutParams> { topMargin = 12.dpToPx }
            imageView?.updateLayoutParams<MarginLayoutParams> {
                height = 48.dpToPx
                width = 48.dpToPx
            }
            imageLayout?.updateLayoutParams<MarginLayoutParams> { height = 48.dpToPx }
        }
    }
     ...
     *
     * @param recyclerOrNested used to determine how far it has scrolled down, if it has not
     *   scrolled past the app bar's height, match the Y to the recyclerView's offset
    ...
    fun <T> updateAppBarAfterY(recyclerOrNested: T?, cancelAnim: Boolean = true) {
        val recyclerView =
            recyclerOrNested as? RecyclerView ?: recyclerOrNested as? NestedScrollView
        if (cancelAnim) {
            yAnimator?.cancel()
        }
        if (lockYPos) return
        ...
        val tabHeight = if (tabsFrameLayout?.isVisible == true) 48.dpToPx else 0
        val shortH =
            if (toolbarMode != ToolbarState.EXPANDED || compactSearchMode) 0f
            else compactAppBarHeight
        val smallHeight = -realHeight + shortH + tabHeight
        val newY =
            when {
                // for smaller devices, when search is active, we want to shrink the app bar and
                // never
                ...
                toolbarMode == ToolbarState.EXPANDED && compactSearchMode -> {
                    MathUtils.clamp(
                        ...
                        -realHeight.toFloat() +
                            top +
                            if (context.isTablet()) minTabletHeight else 0,
        ...
        val alpha =
            (bigHeight + newY * 2) / (bigHeight) +
                0.45f // (realHeight.toFloat() + newY * 5) / realHeight.toFloat() + .33f
        ...
        mainToolbar.alpha =
            MathUtils.clamp(
                (mainToolbar.bottom + mainToolbar.translationY + y - paddingTop) /
                    mainToolbar.height,
                0f,
                1f,
            )
    ...
    fun snapAppBarY(
        ...
        return if (!onFirstItem) {
            yAnimator = animate().y(lastY).setDuration(shortAnimationDuration.toLong())
