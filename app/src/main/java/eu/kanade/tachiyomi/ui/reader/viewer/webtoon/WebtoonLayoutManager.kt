 ...
 * space feature is used which allows setting the image even if the holder is not visible, avoiding
 * (in most cases) black views when they are visible.
 *
...
class WebtoonLayoutManager(activity: ReaderActivity) : LinearLayoutManager(activity) {
    ...
    fun findLastEndVisibleItemPosition(): Int {
        ensureLayoutState()
        @ViewBoundsCheck.ViewBounds
        val preferredBoundsFlag =
            (androidx.recyclerview.widget.ViewBoundsCheck.FLAG_CVE_LT_PVE or
                androidx.recyclerview.widget.ViewBoundsCheck.FLAG_CVE_EQ_PVE)
        ...
        val child =
            if (mOrientation == HORIZONTAL) {
                mHorizontalBoundCheck.findOneViewWithinBoundFlags(
                    fromIndex, toIndex, preferredBoundsFlag, 0)
            } else {
