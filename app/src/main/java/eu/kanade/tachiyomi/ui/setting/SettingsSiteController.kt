    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) =
        screen.apply {
            ...
            preference {
                ...
                onClick {
                    when (
                        mangaDexPreferences.blockedGroups().get().isEmpty() &&
                            angaDexangaDexPreferences.blockedUploaders().get().isEmpty()
                    ) {
                        true -> context.toast(R.string.no_blocked_scanlator)
                        false -> {
                            val blockedScanlators =
                                mangaDexPreferences.blockedGroups().get().map { it to true }
                            val blockedUploaders =
                                angaDexangaDexPreferences.blockedUploaders().get().map { it to false }
