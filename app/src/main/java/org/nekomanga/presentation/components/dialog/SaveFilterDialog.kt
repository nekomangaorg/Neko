 ...
 */
@Composable
fun SaveFilterDialog(themeColorState: ThemeColorState, currentSavedFilters: List<String>, onDismiss: () -> Unit, onConfirm: (String) -> Unit) {
    ...
    CompositionLocalProvider(LocalRippleTheme provides themeColorState.rippleTheme, LocalTextSelectionColors provides themeColorState.textSelectionColors) {
        LaunchedEffect(saveFilterText, currentSavedFilters) {
            if (saveFilterText.isEmpty()) {
                saveEnabled = false
                errorMessage = ""
            } else if (currentSavedFilters.any { it.equals(saveFilterText, true) }) {
                saveEnabled = false
