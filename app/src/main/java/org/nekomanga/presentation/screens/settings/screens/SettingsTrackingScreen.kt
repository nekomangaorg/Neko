...
@Composable
fun SettingsTrackingScreen(modifier: Modifier = Modifier, contentPadding: PaddingValues) {
    LazyColumn(contentPadding = contentPadding, modifier = Modifier.fillMaxWidth()) {
        item {
            IconItem(labelText = UiText.String("Tracking"), icon = Icons.Default.Info, onClick = {})
        }
    }
}
