...
@Composable
fun SettingsSecurityScreen(modifier: Modifier = Modifier, contentPadding: PaddingValues) {
    LazyColumn(contentPadding = contentPadding, modifier = Modifier.fillMaxWidth()) {
        item {
            IconItem(labelText = UiText.String("Security"), icon = Icons.Default.Info, onClick = {})
        }
    }
}
