...
@Composable
fun SettingsAdvancedScreen(modifier: Modifier = Modifier, contentPadding: PaddingValues) {
    LazyColumn(contentPadding = contentPadding, modifier = Modifier.fillMaxWidth()) {
        item {
            IconItem(labelText = UiText.String("Advanced"), icon = Icons.Default.Info, onClick = {})
        }
    }
}
