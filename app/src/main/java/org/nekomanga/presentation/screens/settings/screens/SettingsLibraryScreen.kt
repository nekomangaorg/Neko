...
@Composable
fun SettingsLibraryScreen(modifier: Modifier = Modifier, contentPadding: PaddingValues) {
    LazyColumn(contentPadding = contentPadding, modifier = Modifier.fillMaxWidth()) {
        item {
            IconItem(labelText = UiText.String("Library"), icon = Icons.Default.Info, onClick = {})
        }
    }
}
