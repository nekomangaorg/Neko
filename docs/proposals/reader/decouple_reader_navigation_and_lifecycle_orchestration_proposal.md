...
4. **Divided Concurrency Control:** Concurrency checks (`isScrollingThroughPagesOrChapters` vs `isLoadingAdjacentChapter`) are separated, allowing edge-case races during fast navigation.
...
4. **Replace `requestedPagePosition` with a buffered unidirectional navigation command channel (`ReaderNavCommand`)**.
