...
class DisplayRepository(
    ...
    suspend fun getPage(
        ...
        return when (displayScreenType) {
            ...
            is DisplayScreenType.Tag -> getTagPage(displayScreenType.tag, page)
            is DisplayScreenType.AuthorByName -> getAuthorPage(displayScreenType.authorSearch)
            is DisplayScreenType.AuthorById -> getAuthorManga(page, displayScreenType.authorUUID)
            is DisplayScreenType.GroupByName -> getGroupPage(displayScreenType.groupSearch)
            is DisplayScreenType.GroupById -> getGroupManga(page, displayScreenType.groupUUID)
        }
    }
    ...
    private suspend fun getListPage(listUUID: String): Result<DisplayPageResult, ResultError> {
        return mangaDex
            ...
            .mapBoth(
                success = { listResults ->
                    val displayMangaList = listResults.sourceManga.toDisplayManga(db, mangaDex.id)
