...
## 2024-05-18 - DeepLinkViewModel Testing DI **Learning:** When writing tests for classes in Nekomanga that use Injekt (like ViewModels), `InjektRegistrar` cannot be instantiated directly as it is an interface. Use `Injekt.addSingleton()` directly. **Action:** Next time setting up tests involving `Injekt`, just call `Injekt.addSingleton(mock)` directly instead of trying to override the `Injekt` singleton instance with `InjektRegistrar`.
